...
import { PawPrint, ShoppingCart, User as UserIcon, ChevronDown, Bell } from 'lucide-react';
...
const Navbar = () => {
  ...
  const fetchCounts = useCallback(async (uid: string) => {
    // Cart count
    const { data: cartData } = await supabase.from('cart').select('quantity').eq('user_id', uid);
    if (cartData) setCartCount(cartData.reduce((sum, r) => sum + Number((r as { quantity: number }).quantity || 0), 0));
  ...
  useEffect(() => {
    ...
    const initUser = async () => {
      ...
      if (user) {
        ...
        const notifsChannel = supabase.channel(`notifs-badge-${user.id}`)
          .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications', filter: `user_id=eq.${user.id}` },
            () => fetchCounts(user.id))
          .subscribe();

        const interval = setInterval(() => fetchCounts(user.id), 10000);
        const onVis = () => document.visibilityState === 'visible' && fetchCounts(user.id);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          >
            ...
            <Link href="/" className="flex items-center space-x-2 group z-50">
              ...
              <motion.span className="text-2xl font-bold text-[#f5f5dc] group-hover:text-[#FF8A65] transition-colors duration-200" style={{ letterSpacing: '0.2em' }}>
                PETZONEE
              </motion.span>
            </Link>
